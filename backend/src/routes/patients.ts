...
router.post('/patients', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, patientId: patient.id, msg: 'Patient was successfully added' });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, msg: 'An error occurred while adding the patient.' });
  }
});
