...
const userSchemaLog = Joi.object().keys({
	email: Joi.string().email().required(),
	password: Joi.string().required(),
});
...
router.post('/login', (req, res) => {
	// Joy Validation
	const result = userSchemaLog.validate(req.body);
	if (result.error) {
		res.status(422).json({
			success: false,
			msg: `Validation err: ${result.error.details[0].message}`,
	...
	userRepository.findOne({ email }).then((user) => {
		...
		if (!user.firstname || !user.name) {
			return res.json({ success: false, msg: 'No valid name' });
		}
...
router.post('/edit', checkToken, async (req, res) => {
    const {
        userID, email, mobile, titre, speciality, RPPS, nameInstitution,
        address, postalCode, city, country, nbInstitution
    } = req.body;
